...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        button_seed.addActionListener(e -> {
            if (Objects.equals(textField.getText(), "")) tips_null("seed", "114514");
            else {
                GUI.setTitle("SlimeChunkFinder    --editing--");
                seed[0] = Long.getLong(textField.getText());
                Tips_SeedInputSuccess(seed[0]);
            }
        });
        panel.add(label);
        panel.add(textField);
        panel.add(button_seed);
        ...
        button_loc.setBounds(450, 115, 100, 30);
        button_loc.addActionListener(e -> {
            if (Objects.equals(text_x.getText(), "") || Objects.equals(text_y.getText(), ""))
                ...
                if (Math.abs(Integer.parseInt(text_x.getText())) > 300000 || Math.abs(Integer.parseInt(text_y.getText())) > 300000)
                    new Warn().NOT_VANILLA("Range must smaller than 300k");
                else {
        ...
        button_distance.addActionListener(e -> {
            if (Objects.equals(text_distance.getText(), "")) tips_null("distance", "1000");
                ...
                if (Integer.parseInt(text_distance.getText()) >= 4150) {
                    new Warn().NOT_VANILLA("distance." + text_distance.getText() + rua);
        ...
        panel.add(label1);
        panel.add(text_distance);
        panel.add(button_distance);
        JButton all = new JButton("AllConfirm");
        all.setBounds(450, 280, 100, 50);
        all.setBorder(BorderFactory.createLineBorder(Color.cyan, 3, false));
        all.addActionListener(e -> {
            if (Objects.equals(text_distance.getText(), "")) tips_null("distance", "1000");
                ...
                if (Integer.parseInt(text_distance.getText()) >= 4150) {
                    new Warn().NOT_VANILLA("distance." + text_distance.getText() + rua);
                    text_distance.setText("");
                } else {distance[0] = Integer.parseInt(text_distance.getText());
                    Tips_DistanceInputSuccess(distance[0]);}}
            if (Objects.equals(text_x.getText(), "") && Objects.equals(text_y.getText(), ""))
                tips_null("x", "0");
            else if (Objects.equals(text_y.getText(), "")) tips_null("y", "0");
            else if (Objects.equals(text_x.getText(), "")) tips_null("x", "0");
            else {if (Math.abs(Integer.parseInt(text_x.getText())) > 300000 ||
                    Math.abs(Integer.parseInt(text_y.getText())) > 300000)
                    new Warn().NOT_VANILLA("Range must smaller than 300k");
                else {center.x = Integer.parseInt(text_x.getText());
                    center.y = Integer.parseInt(text_y.getText());
                    Tips_PointInputSuccess(center.x, center.y);}}
            if (Objects.equals(textField.getText(), "")) tips_null("seed", "114514");
            else {seed[0] = Long.getLong(textField.getText());
                Tips_SeedInputSuccess(seed[0]);}});
        panel.add(all);
        JLabel label12 = new JLabel("");
        label12.setBorder(BorderFactory.createLineBorder(Color.BLACK, 1));
        label12.setBounds(0, 260, 600, 1);
        panel.add(label12);
        JButton cal = new JButton("Start Calculation");
        cal.setBounds(140, 280, 120, 60);
        cal.setBackground(Color.PINK);
        cal.setBorder(BorderFactory.createLineBorder(Color.GRAY, 1, true));
        cal.addActionListener(e -> {
            long start = System.currentTimeMillis();
            GUI.setTitle("SlimeChunkFinder     --Calculating--");
            cal(seed[0], center, distance[0]);
            long cost = new timer().end(start);
            GUI.setTitle("SlimeChunkFinder");
            time(cost);
        });
        panel.add(cal);
        JButton info = new JButton("Info");
        info.setBounds(20, 290, 70, 40);
        info.setBorder(BorderFactory.createEtchedBorder());
        info.addActionListener(e -> info());
        panel.add(info);
        JButton exit = new JButton("EXIT");
        exit.setBounds(310, 290, 70, 40);
    ...
    public static void cal(long seed, Point center, int distance) {
        ...
        frame.setBounds(500, 400, 700, 800);
        frame.setResizable(false);
        JPanel panel = new JPanel();
        panel.setLayout(null);
        JTextArea area = new JTextArea(str);
        area.setLineWrap(true);
        area.setBounds(10, 10, 670, 780);
        area.setBorder(BorderFactory.createLineBorder(Color.PINK, 5));
